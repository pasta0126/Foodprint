...
user set their theme preference (system / light / dark) and sign out. The theme
preference SHALL take effect immediately and persist per the design-system theme
rules. Sign-out from the profile page SHALL end the current session and return the
user to the signed-out entry point.
...
- **WHEN** a signed-in user looks for where to change language, theme, or sign out
- **THEN** all three are on the profile page, reachable from the header identity control
